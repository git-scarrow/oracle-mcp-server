...
// Block reasons auto-generated by the dispatch tooling for transient/computed
// conditions (mirrors notion-forge's _AUTO_BLOCK_PREFIXES). The V18 gate ignores
// these — they are re-derived live from the V15-V20 conditions each pass — and
// only fires on human- or preflight-set reasons, so a resolved transient
// condition does not leave an item permanently non-dispatchable.
export const AUTO_BLOCK_PREFIXES = [
...
export function isAutoBlockReason(reason) {
  return typeof reason === 'string' && AUTO_BLOCK_PREFIXES.some((p) => reason.startsWith(p));
}
